...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!Text) return message.channel.send(`<a:idk:873833709864902676> | Please Give anime character to search information about that!`);

        if (Text.length > 200) return message.channel.send(`Text Limit - 200`);

        let Msg = await message.channel.send(`**Searching It For You <a:idk:873833709864902676> please wait for while**`);
