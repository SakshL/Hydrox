...
module.exports.run = async (client, message) => {
    ...
    if (cmd.length === 0){
      ...
      if(matchedPrefix.includes(client.user.id)) {
        ...
        .setDescription(`<:Spotify:878694621612478515> Enjoy Listening To Music By Using The \`-help\` Command.`)
        .addField('Invite Me',`
    ...
    if (command){
        ...
        if (timestamps.has(message.author.id)) { //if the user is on cooldown
          ...
          if (now < expirationTime) { //if he is still on cooldonw
            ...
            not_allowed = true;
            return message.channel.send(`<:no:878695788040056852> Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`); //send an information message
          }
        }
        ...
        if(command.memberpermissions) {
          if (!message.member.hasPermission(command.memberpermissions)) {
            ...
            message.channel.send(new Discord.MessageEmbed()
              ...
              .setTitle("<:no:878695788040056852> You are not allowed to run this command!")
        ...
        if(command.parameters) {
          if(command.parameters.type == "music"){
            ...
            if (!channel) {
              ...
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:878695788040056852> You need to join a voice channel.`)
              );
            }
            ...
            if(!message.guild.me.permissionsIn(channel.id).toArray().includes("CONNECT")){
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:878695788040056852> I am not allowed to connect to this Channel.`)
              );
            }
            if(!message.guild.me.permissionsIn(channel.id).toArray().includes("SPEAK")){
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:878695788040056852> I am not allowed to speak in this Channel to this Channel.`)
              );
            }
            if(channel.userlimit && channel.userlimit != 0 && channel.full)
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:878695788040056852> Your Voice Channel is full, I can't join!`)
              );
            ...
            if(!channel.joinable)
              return message.channel.send(new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setTitle(`<:no:878695788040056852> I am not able to join your Channel!`)
              );
            ...
            if(command.parameters.activeplayer){
              if (!player){
                ...
                return message.channel.send(new MessageEmbed()
                  .setColor(ee.wrongcolor)
                  .setTitle(`<:no:878695788040056852> There is nothing playing`)
                );
              }
              if (!mechannel){
                if(player) try{ 
